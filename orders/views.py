...
from .models import DepartmentRequest  # Adjust if your models are in another app
...
def submit_request(request):
    ...
    try:
        ...
        return render(request, 'department/submit_request.html', context)
...
def qm_dashboard(request):
    ...
    return render(request, 'qm/qm_dashboard.html', context)
...
def qm_forward(request):
    ...
    return render(request, 'qm/qm_forward.html', context)
...
def view_feedbacks(request):
    ...
    return render(request, 'qm/qm_feedback.html', {'feedbacks': feedbacks})
...
def co_dashboard(request):
    ...
    return render(request, 'co/co_dashboard.html', context)
...
def my_requests_view(request):
    my_requests = DepartmentRequest.objects.filter(submitted_by=request.user)
    return render(request, 'department/my_requests.html', {'my_requests': my_requests})


@login_required
def notifications_view(request):
    qm_responses = QMResponse.objects.filter(request__submitted_by=request.user).order_by('-response_date')
    unread_responses_count = qm_responses.filter(is_read_by_department=False).count()
    
    return render(request, 'department/notifications.html', {
...
def feedback_view(request):
    feedbacks = COFeedback.objects.all().order_by('-date_sent')
    return render(request, 'department/feedback.html', {'feedbacks': feedbacks})


@login_required
def qm_incoming_requests(request):
    incoming_requests = DepartmentRequest.objects.filter(status='pending').order_by('-date_submitted')
    return render(request, 'qm/incoming_requests.html', {
        'incoming_requests': incoming_requests
    })

def forwarded_requests_view(request):
    forwarded_requests_list = DepartmentRequest.objects.filter(status='forwarded').order_by('-qmnote__forwarded_date')
    context = {
        'forwarded_requests_list': forwarded_requests_list,
    }
    return render(request, 'qm/forwarded_requests.html', context)


def co_decisions_view(request):
    # Filter requests with CO decisions and status awaiting response (you can adjust this logic)
    co_decided_requests = DepartmentRequest.objects.filter(
        status__in=['approved', 'rejected']
    ).select_related('codecision', 'department')

    context = {
        'co_decided_requests': co_decided_requests,
    }
    return render(request, 'qm/co_decisions.html', context)


def co_feedback_view(request):
    # Query all CO Feedback objects, order by most recent first
    co_feedbacks = COFeedback.objects.all().order_by('-date_sent')
    
    # Count unread feedbacks for badge notification
    ...
    return render(request, 'qm/co_feedback.html', context)

def review_requests_view(request):
    review_list = DepartmentRequest.objects.filter(status='forwarded').select_related('department', 'submitted_by')
    return render(request, 'co/review_requests.html', {'review_list': review_list})


def decision_history_view(request):
    # Fetch recent decisions, ordered by decision_date descending
    recent_decisions = CODecision.objects.select_related('request', 'request__department', 'request__submitted_by').all()
    
    context = {
        'recent_decisions': recent_decisions,
    }
    return render(request, 'co/decision_history.html', context)
...
def send_feedback_view(request):
    ...
    return render(request, 'co/send_feedback.html')
